...
def pop_galaxy(target: celbd.Galaxy):
        for i in range(target.star_count):
            new_star = celbd.Star(position=smaths.Vec2(random.randint(0,956), random.randint(0,537)), colour=graphdefs.Colour3(random.randint(0,255),random.randint(0,255),random.randint(0,255)), scale=random.uniform(0.5,2), index=i, name=wordlists.generate_word_from_syllables(3,6))

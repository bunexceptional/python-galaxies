...
def init_pyglet(galaxy:celbd.Galaxy):
    ...
    for star in range(len(galaxy.star_list)):
        ...
        new_sprite.update(scale=index_star.scale)
        star_sprites.append(new_sprite)
        new_label = pyglet.text.Label(index_star.name, font_name='Verdana', font_size=8, x=index_star.position.x, y=index_star.position.y - 2, anchor_x='center', anchor_y='top', batch=star_batch)
